...
export class PoliticalMapManager {
  ...
  addBoundaries(geojson,{color=new Color3(.48,.62,.55),y=.85}={}){
    for(const f of geojson?.features||[]){for(const ring of lineRings(f)){if(!intersects(ring))continue;const points=ring.map(([lng,lat])=>geoToWorld(lng,lat,y));if(points.length<2)continue;const line=MeshBuilder.CreateLines(`limite-${this.items.length}`,{points,updatable:false},this.scene);line.color=color;line.alpha=.95;line.isPickable=false;line.renderingGroupId=2;this.items.push(line);}}
  }
