...
import { horaActual } from "../lib/campoClima";
import { tiempoRelativo } from "../lib/tiempoRelativo";
import WeatherIcon from "./WeatherIcon";
import WeatherDebugPanel from "./WeatherDebugPanel";
import LayerPanel from "./LayerPanel";

const BabylonMap = forwardRef(function BabylonMap({ municipiosGeojson, mundoGeojson, paisesLabels, provincias, provinciasLabels, pronostico, clima, titulo, publicadoEn, interactive=true }, ref) {
  ...
  useEffect(()=>{if(ready&&mundoGeojson)managerRef.current.setPolitical({world:mundoGeojson,provincias,countryLabels:paisesLabels,provinceLabels:provinciasLabels,municipios:municipiosGeojson});},[ready,mundoGeojson,provincias,paisesLabels,provinciasLabels,municipiosGeojson]);
  useEffect(()=>{if(ready&&clima)managerRef.current.setClimateGrid(clima,hora);},[ready,clima,hora]);
  ...
  return <div className="base-map babylon-map">
    ...
    {interactive&&clima&&(
      <LayerPanel
        capas={capas}
        onToggle={(id)=>setCapas((c)=>{const next={...c,[id]:!c[id]};managerRef.current?.setClimateLayer(id,next[id]);return next;})}
        horas={clima.horas}
        hora={hora}
        horaAhora={horaActual(clima)}
        onHora={setHora}
      />
    )}
