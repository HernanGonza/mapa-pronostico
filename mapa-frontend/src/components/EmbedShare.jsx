...
export default function EmbedShare({ path, title, kind = 'mapa' }) {
  const [copiado, setCopiado] = useState("");
  const [error, setError] = useState("");
  const [vistaAbierta, setVistaAbierta] = useState(false);
  const historico = kind === 'historico';
  const url = `${window.location.origin}${path}`;
  const code = `<iframe src="${url}" title="${title}" width="100%" height="${historico ? 900 : 720}" style="border:0" loading="lazy"></iframe>`;
  ...
  return <div className="risk-share"><h2>{historico ? 'Cuadro estadístico para el sitio web' : 'Mapa para el sitio web'}</h2>
    <p className="admin-panel__hint">{historico ? 'Este cuadro está disponible siempre y consulta los datos actuales al abrirse. Copiá el enlace o el iframe para mostrarlo en el sitio.' : 'El enlace y el código muestran la última versión publicada.'}</p>
    <label>{historico ? 'Enlace al histórico público' : 'Enlace al mapa publicado'}<input readOnly value={url} onFocus={e => e.target.select()} /></label>
    <a href={url} target="_blank" rel="noreferrer">{historico ? 'Abrir cuadro estadístico ↗' : 'Abrir mapa publicado ↗'}</a>
